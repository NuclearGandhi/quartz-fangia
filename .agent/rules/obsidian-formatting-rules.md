...
- When referencing a numbered equation that lives in another file, include an Obsidian link to that source (for example `[[Course_000#Section|תיאור]] $\text{(ID)}$`).
...
- Include proper captions for all figures using the format `>Figure caption. [[Course_000 Course Name#ביבליוגרפיה|(Author et al., Year)]]`. In the case of excalidraw svg files, there is no need to include a source reference.
...
- When referring to an existing figure, add an inline link to its anchor (for example `[[#^figure-anchor|איור]]`).
- Use consistent image sizing (typically 600px width for main figures), unless the figure is an excalidraw svg file, in which case use the original size of the file.
- Leave appropriate space and context for figures to be inserted
- For figures use the format: `![[imagename.png|bookhue|size]]^figure-insert-name-here`
