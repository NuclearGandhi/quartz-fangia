...
export async function handleConvert(argv) {
  ...
  // Use separate cache file for convert
  const convertCacheFile = cacheFile.replace('transpiled-build.mjs', 'transpiled-convert.mjs')

  // Set up the esbuild context for convert.ts
  const ctx = await esbuild.context({
    entryPoints: [convertFp],
    outfile: convertCacheFile,
    bundle: true,
    keepNames: true,
    minifyWhitespace: true,
    minifySyntax: true,
  ...
  const result = await ctx.rebuild().catch((err) => {
    console.error(`${chalk.red("Couldn't parse Quartz configuration:")} ${convertFp}`)
    console.log(`Reason: ${chalk.grey(err)}`)
    process.exit(1)
  })

  // Import the convert module using the separate cache file
  const { default: convertQuartz } = await import(`../../${convertCacheFile}?update=${randomUUID()}`)

  // Call the convert function
  await convertQuartz(argv)
  ctx.dispose()
}
